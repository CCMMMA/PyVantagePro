...
class VantagePro2(object):
    ...
    ESC = '\x1b'
    OK = '\n\rOK\n\r'
    ZERO_VALUE_ALARM_JSON_KEYS = frozenset((
        "AlarmInFallBarTrend",
        "AlarmInRisBarTrend",
        "AlarmInLowTemp",
        "AlarmInHighTemp",
    ...
    BYTE_MAX_SENTINEL_JSON_KEYS = frozenset((
        "ExtraTemps01",
        "ExtraTemps02",
        "ExtraTemps03",
        "ExtraTemps04",
    ...
    def meta(self):
        '''Return metadata as {variable_name: {internal_unit, si_unit}}.'''
        return {
            key: {
                "internal_unit": self._meta_internal_units(key),
                "si_unit": self._meta_si_units(key),
            }
    ...
    def _mapping_to_json_payload(self, data):
        ...
        for key, value in data.items():
            ...
            if key in self.ZERO_VALUE_ALARM_JSON_KEYS and value == 0:
                continue
            # Drop known optional sensor fields when hardware reports 255.
